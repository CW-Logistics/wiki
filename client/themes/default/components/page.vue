            ...
            v-card.page-toc-card.mb-5(v-if='tocDecoded.length')
              ...
              v-list.pb-3(dense, nav, :class='$vuetify.theme.dark ? `darken-3-d3` : ``')
                template(v-for='(tocItem, tocIdx) in tocDecoded')
                  template(v-if='tocItem.anchor && tocItem.anchor.startsWith(`---:`)')
                    v-divider.my-1
                    .toc-separator.px-4.py-1.caption.grey--text(:class='$vuetify.theme.dark ? `text--lighten-1` : `text--darken-2`') {{tocItem.anchor.slice(4)}}
                  template(v-else)
